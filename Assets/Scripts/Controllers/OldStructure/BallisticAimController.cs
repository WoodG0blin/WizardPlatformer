...
namespace WizardsPlatformer
{
    public class BallisticAimController : AimController
    {
        ...
        public BallisticAimController(BasicView view, float maxDistance = 0) : base(view, maxDistance) {}
        protected override void OnUpdate()
        {
            ...
            if(InDistance())
            {
                for (int i = 89; i > 20; i--)
                {
                    float dx = Mathf.Abs(Player.position.x - _view.transform.position.x);
                    float dy = (Mathf.Tan(i / 180f * Mathf.PI) * dx) - ((2f * 9.8f * dx * dx) / (2 * _speed * _speed * Mathf.Cos(i / 180f * Mathf.PI) * Mathf.Cos(i / 180f * Mathf.PI)));

                    if (Mathf.Abs(dy + _view.transform.position.y - Player.position.y) < 0.2f)
                    {
                        _angle = (90 - i) * (int)Mathf.Sign(Player.position.x - _view.transform.position.x);
                        break;
                    }
                }
            }
